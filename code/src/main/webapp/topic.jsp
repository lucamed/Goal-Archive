...
<section class="content-section">
    <div class="container">
        ...
        <div class="alert alert-error">
            ...
            ⚠️ Errore: il commento non può essere vuoto.
            <% } else if ("non_autenticato".equals(request.getParameter("errore"))) { %>
            ⚠️ Devi essere autenticato per commentare.
            <% } else { %>
            ⚠️ Errore nella pubblicazione del commento.
            <% } %>
        </div>
        <% } %>

        <!-- Form Commento (solo per utenti registrati) -->
        <% if (utente != null) { %>
        ...
        <div class="alert alert-info">
            👉 <a href="${pageContext.request.contextPath}/login.jsp">Effettua il login</a> per partecipare alla discussione.
        </div>
        <% } %>

        <!-- Lista Commenti -->
        <div class="commenti-section">
            <h3>💬 Commenti (<%= commenti != null ? commenti.size() : 0 %>)</h3>

            <% if (commenti != null && !commenti.isEmpty()) { %>
            <% for (Commento commento : commenti) { %>
            <div class="commento-card">
                <div class="commento-header">
                    <strong>👤 <%= commento.getNomeUtente() != null ? commento.getNomeUtente() : "Utente" %></strong>
                    <span class="commento-date">
                                    <%= commento.getDataPubblicazione() != null ? sdf.format(commento.getDataPubblicazione()) : "" %>
                                </span>
                </div>
