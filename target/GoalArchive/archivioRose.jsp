...
<%
    ...
    // ✅ Mappa per verificare quali giocatori sono preferiti
    Map<Integer, Boolean> preferitiMap = new HashMap<>();

    if (utente != null) {
        ManagerPreferiti managerPreferiti = new ManagerPreferiti();
        try {
            List<Preferito> preferiti = managerPreferiti.getPreferitiPerTipo(utente.getEmail(), "calciatore");
            for (Preferito p : preferiti) {
                preferitiMap.put(p.getIdRiferimento(), true);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    <div class="ruolo-section">
        ...
        <div class="giocatori-list">
            <%
                ...
                // ✅ Controlla se il giocatore è nei preferiti
                boolean isPreferito = preferitiMap.containsKey(giocatore.getIdCalciatore());
            %>
                ...
                <div class="giocatore-info">
                    <div style="display: flex; align-items: center; gap: 0.75rem;">
                        ...
                        <form action="${pageContext.request.contextPath}/gestionePreferiti"
                            ...
                            <input type="hidden" name="idRiferimento" value="<%= giocatore.getIdCalciatore() %>">

                            <% if (isPreferito) { %>
                            <input type="hidden" name="azione" value="rimuovi">
                            <button type="submit" class="btn-favorite-small" title="Rimuovi dai preferiti">⭐</button>
                            <% } else { %>
                            <input type="hidden" name="azione" value="aggiungi">
                            <button type="submit" class="btn-favorite-small" title="Aggiungi ai preferiti">☆</button>
                            <% } %>
                        </form>
                        <% } %>
                    </div>
